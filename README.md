...
2. To start the program download the repository and launch the `main.py` script. The only requirements are `tkinter` and `json` external libraries to be installed beforehand (on Windows they are installed by default with python).
